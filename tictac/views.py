...
class TicTacToe():
    ...
    def is_winner(self,player):
        ...
        if [self.grid[i][i] for i in range(3)] == winner[player]: # This list comp transposes diag1 to row
                return True
        if [self.grid[i][i] for i in range(2,-1,-1)] == winner[player]: # This list comp transposes diag2 to row
                return True
...
def is_winner(grid,player):
    ...
    if [grid[i][i] for i in range(3)] == winner[player]: # This list comp transposes diag1 to row
            return True
    if [grid[i][i] for i in range(2,-1,-1)] == winner[player]: # This list comp transposes diag2 to row
            return True
...
def gridclick(request):
    ...
    if request.method == 'GET':
        ...
        if clicked:
            ...
            if is_move_valid(request.session['grid'],clicked, player):
